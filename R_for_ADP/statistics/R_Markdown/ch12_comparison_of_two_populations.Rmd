...
#### 12.5.12 한 과일 재배자는 새로운 살충제가 곤충으로 인한 손해를 줄일 수 있다는 주장을 확인하고자 한다. 따라서 과일 재배자는 그의 과수원에서 나무 27그루를 이용하여 실험을 하였는데, 나무 12그루에는 새로운 살충제로 처리하고 15그루에는 기존의 살충제로 처리하였다. 그 결과 과일 생산량의 자료가 자료와 같이 얻어졌다고 하자. 이들 자료는 새로운 살충제로 사용하는 것이 더 많은 산출량을 보이고 있음을 증명하는가? 

##### (1) 필요한 가정에 대해 언급하라

##### (2) 유의수준 0.05에서 검정을 실시하라.

##### (3) 새로운 살충제와 기존의 살충제 간의 평균 산출량의 차에 대한 95% 신뢰구간을 구하라.

##### (4) 새로운 살충제와 기존의 살충제 각각에 대하여 평균 산출량에 대한 90% 신뢰구간을 구하라.
...
# 4번
lowerbound_1 = (x_bar) + qt(0.05, df=n_1-1) * s_1 * sqrt(1/n_1)
upperbound_1 = (x_bar) + qt(0.95, df=n_1-1) * s_1 * sqrt(1/n_1)
lowerbound_2 = (y_bar) + qt(0.05, df=n_2-1) * s_2 * sqrt(1/n_2)
upperbound_2 = (y_bar) + qt(0.95, df=n_2-1) * s_2 * sqrt(1/n_2)
answer_4_1 = paste("(", lowerbound_1, ",", upperbound_1, ")")
answer_4_2 = paste("(", lowerbound_2, ",", upperbound_2, ")")
...
#### 12.5.16 1970년대 초에 학생들은 스트리킹(streaking)을 하였다. 교정에서 첫 번째 스트리킹이 있은 후 2주일 동안 스트리킹의 허용을 요구하는 16명의 남학생(집단 1)과 반대하는 16명의 남학생(집단 2)을 대상으로 심리 테스트를 실시하였다. 심리 테스트에 대한 점수는 외향성을 나타낸다.

##### (1) 모평균 차에 대한 95% 신뢰구간을 구하라. 두 집단이 평균에 차이가 있는가?

##### (2) 스트리킹의 허용을 요구하는 학생이 반대하는 학생과 성격이 다를 수 있다. 이러한 가능성에 대한 분석의 결론은 어떻다고 할 수 있는가?

```{r}
data = read.csv("../Data/exercise_12_5_16.csv", row.names = 1)
...
p_value = 2 * (1 - pt(abs(t), df = n_1 + n_2 - 2))
...
#### 12.5.18 두 개의 처리방법을 비교하기 위해 9쌍의 실험재료에 짝비교 실험을 하였다. 처리1과 처리2의 반응 차에 대한 기록을 하였는데 자료와 같았다.

##### (1) 두 개의 처리방법에 대한 평균 반응값에 차이가 있는가? 유의수준 0.02에서 검정하라.

##### (2) 두 개의 처리방법에 대한 평균 반응 차의 90% 신뢰구간을 구하라.

```{r}
data = read.csv("../Data/exercise_12_5_18.csv")$x
...
# 2번
alpha = 0.1
lowerbound = (x_bar) + qt(alpha/2, df=n-1) * s * sqrt(1/n)
upperbound = (x_bar) + qt(1-alpha/2, df=n-1) * s * sqrt(1/n)
answer_2 = paste("(", lowerbound, ",", upperbound, ")")
...
#### 12.5.20 표준 비료에 특별한 화학 첨가물을 사용하는 것이 식물의 성장을 촉진시키는가를 알아보기 위해서 실험을 하고자 한다. 실험을 할 10곳의 위치가 선정되었다. 각 위치에서 근접한 두 그루의 식물을 대상으로 실험을 수행하는데, 한 그루에는 표준 비료만을 사용하고 다른 한 그루에는 표준 비료에 특별한 화학 첨가물을 사용해서 실험을 하고자 한다. 4주 후에 그동안 식물이 자란 길이(cm)를 재어보았다. 아래의 자료는 표준 비료에 특별한 화학 첨가물을 사용하는 것이 식물의 성장을 촉진시킨다는 주장을 뒷받침하는가? 

##### (1) 필요한 가정을 언급하고 유의수준 0.05에서 검정하라. 

##### (2) 두 모평균의 차 $\delta$에 대한 95% 신뢰구간을 구하라.

```{r}
data = read.csv("../Data/exercise_12_5_20.csv")

# 1번
data$D = data$N - data$Y
n = length(data$D)
x_bar = mean(data$D)
s = sd(data$D)
alpha = 0.05

t = (x_bar - 0) / (s * sqrt(1/n))
...
assumption = paste(
  "1. 쌍 간 독립성: \n",
  "   각 쌍은 서로 독립이나, 각 쌍 내에서 N과 Y는 높은 상관관계를 가진다.\n",
  "   2. 차이의 정규성:\n",
  "   두 표본의 차이(D = N-Y)가 정규분포를 따라야 한다.(모집단이 정규분포)\n",
  "   3. 측정의 동일성:\n",
  "   동일한 방법과 조건에서 정확하게 측정되어야 한다.\n"
)
...
#### 12.5.22 새로운 교육방법이 현재 사용하고 있는 방법보다 초등학생의 읽기 능력을 더 향상시킬 수 있다고 기대한다. 이 추측을 증명하기 위해 16명의 학생을 임의로 8명으로 나누어서 두 개의 그룹으로 만들었다. 한 그룹에는 현재 사용하고 있는 방법으로, 또 다른 그룹에는 새로운 교육방법을 사용하여 교육을 실시하였다. 읽기 시험에 대한 학생들의 점수는 자료와 같다. 묘평균점수의 차에 대한 결론을 얻기 위하여 가설검정과 신뢰구간의 방법을 이용하라. ($\alpha=0.05$)

```{r}
data = read.csv("../Data/exercise_12_5_22.csv")
n = nrow(data)
data$d = data$current - data$new
x_bar = mean(data$d)
s = sd(data$d)
alpha = 0.05
...
#### 12.5.24 복부비만을 줄여 주는 두 종류의 운동방법의 효율성을 비교하기 위해 20명을 대상으로 실험하였다. 임의로 10명을 선택하여 한 운동방법을 택하게 하고 나머지 10명은 다른 운동방법을 택하게 하였다. 5주 후에 복부 측정에서의 감량이 자료와 같았다.

##### (1) 이들 자료는 운동방법 2가 더 효율적이라는 주장을 지지하는가?

##### (2) 두 운동방법에 따른 복부 감량의 평균차에 대한 95% 신뢰구간을 구하라.

```{r}
data = read.csv("../Data/exercise_12_5_24.csv", row.names = 1)

x_bar = data$workout_1[1]
y_bar = data$workout_2[1]
s_1 = data$workout_1[2]
s_2 = data$workout_2[2]
n_1 = 10
n_2 = 10
s_p = sqrt( ((n_1-1) * s_1^2 + (n_2-1) * s_2^2) / (n_1+n_2-2) )
alpha = 0.05
...
#### 12.5.26 인공강우를 위해 구름에 뿌리는 약품이 우박의 발생빈도를 줄일 수 있는지에 대한 실험을 하였다. 우박이 잦은 지형의 지역에서 구름이 많은 날 중에서 50일은 구름에 약품을 뿌리고, 165일은 구름에 아무 것도 뿌리지 않고 관측하였다. 

##### (1) 제시된 자료는 구름에 약품을 뿌리는 것이 우박의 발생빈도를 줄일 수 있다는 추측을 뒷받침하는가?

##### (2) 구름에 약품을 뿌리는 경우와 뿌리지 않는 경우에 우박의 발생비율의 차에 대한 90% 신뢰구간을 구하라.

```{r}
data = read.csv("../Data/exercise_12_5_26.csv", row.names = 1)

n_1 = sum(data$chemical)
n_2 = sum(data$no_chemical)
p_1 = data$chemical[1] / n_1
p_2 = data$no_chemical[1] / n_2
p_hat = sum(data['hail', ]) / sum(data)
alpha = 0.05
...
# 2번 
lowerbound = (p_1 - p_2) + qnorm(alpha) * sqrt(p_1*(1-p_1) / n_1 + p_2*(1-p_2) / n_2)
upperbound = (p_1 - p_2) + qnorm(1-alpha) * sqrt(p_1*(1-p_1) / n_1 + p_2*(1-p_2) / n_2)
...
#### 12.5.28 금연패치를 이용하는 것이 금연에 효과가 있는가를 알아보려고 실험을 하였다. 금연을 원하는 80명의 흡연자들 중에서 임의로 42명을 뽑아서 금연패치를 사용하게 하고 다른 38명의 흡연자들에겐 가짜패치를 사용하게 하였다. 4주 후에 금연패치 사용자 중 27명이, 가짜패치 사용자 중 14명이 담배를 끊었다. 이들 결과로부터 금연패치 사용이 담배를 끊게 하는 비율을 높인다고 할 수 있는가? P-값을 이용하여 답하라.
...
#### 12.5.30 100명의 신장병 병력이 있는 환자와 100명의 보통 환자에게 폐렴 항생제를 투여하였다. 그리고 38명의 신장병 병력이 있는 환자와 21명의 보통환자에게서 알레르기 반응이 나타났다.

##### (1) 이들 자료는 항생제에 대한 알레르기 반응의 비율이 보통 환자보다 신장병 병력이 있는 환자에게서 더 높다고 할 수 있는가?

##### (2) 모비율의 차에 대한 95% 신뢰구간을 구하라.
